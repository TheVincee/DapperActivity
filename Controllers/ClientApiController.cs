...
namespace Dapper.Controllers
{
    ...
    {
        private readonly SqliteConnection _connection = new SqliteConnection("Data Source=UrsalUtangan.db");
        ...
        {
            const string query = "INSERT INTO Clients (ClientName, Address) VALUES (@ClientName, @Address); SELECT * FROM Clients ORDER BY Id DESC LIMIT 1";
        ...
        {
            const string query = "UPDATE Clients SET ClientName = @theClientName, Address = @theAddress WHERE Id = @Id; SELECT * FROM Clients WHERE Id = @Id LIMIT 1";
